...
class Pago extends Conectar
{
    public function get_pago()
    {
        ...
        foreach ($resultado as $d) {
            $Array[] = [
                'pago_id' => (int)$d->pago_id, 'total' => $d->total,'tbl_reserva_reserva_id' => (int)$d->tbl_reserva_reserva_id,
                'tbl_tarjeta_tarjeta_id' => (int)$d->tbl_tarjeta_tarjeta_id
            ];
        }
        return $Array;
    }
    ...
    public function get_pago_x_id($pago_id)
    {
        ...
        parent::set_names();
        $sql = "SELECT * FROM tbl_pago WHERE pago_id = ?;";
        $sql = $conectar->prepare($sql);
        $sql->bindValue(1, $pago_id);
        $sql->execute();
        $resultado = $sql->fetch(PDO::FETCH_OBJ);
        $Array = $resultado ? [
            'pago_id' => (int)$resultado->pago_id, 'total' => $resultado->total,'tbl_reserva_reserva_id' => (int)$resultado->tbl_reserva_reserva_id,
                'tbl_tarjeta_tarjeta_id' => (int)$resultado->tbl_tarjeta_tarjeta_id
        ] : [];
        return $Array;
    }
    ...
    public function delete_pago($pago_id)
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "DELETE FROM `tbl_pago` WHERE pago_id = ?;";
        $sql = $conectar->prepare($sql);
        $sql->bindValue(1, $pago_id);
        $resultado['estatus'] = $sql->execute();
        return $resultado;
    ...
    public function get_info_reservacion_tarjetas()
    {
        $conectar = parent::conexion();
        parent::set_names();
        $sql = "SELECT t1.registro_id,
                ...
                WHERE t2.tbl_registro_registro_id = t1.registro_id  AND NOT EXISTS (SELECT * FROM tbl_pago AS t5 WHERE t5.tbl_reserva_reserva_id = t2.reserva_id
                )) AS Reservaciones,
                ...
                WHERE t4.tbl_registro_registro_id = t1.registro_id) AS Tarjetas
